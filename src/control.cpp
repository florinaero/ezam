...
Control::Control(int width_win, int height_win, int size) : m_sp_window(std::make_shared<sf::RenderWindow>(sf::VideoMode(width_win, height_win), "ezam", sf::Style::Default)),
                                                            m_input_size(size)
{
...
void Control::run()
{
    ...
    std::shared_ptr<Grid> sp_grid = std::make_shared<Grid>(Grid(m_input_size, m_sp_window));
    Control::initialize();
    setTitle();
...
sf::Text Control::setStageName(std::string name)
{
    ...
    banner[2].position = sf::Vector2f(x_pos + bounds.width, y_pos + 2 * bounds.height);
